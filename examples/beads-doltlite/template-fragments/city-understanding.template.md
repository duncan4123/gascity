...
The `gc beads-doltlite build` command is pack-managed. It still requires an existing `gc` binary to run the city and dispatch pack commands, then it builds and installs libdoltlite-linked replacements to `<beads-doltlite>/bin/bd` and `<gascity>/bin/gc` by default. Use `gc beads-doltlite build all` for both binaries, or `--bd-output` / `--gc-output` to install them somewhere else.
