...
if ! (
  cd "$city_root"
  GC_DOLTLITE_READ_FASTPATH=1 timeout 10s gc beads show "$candidate" >"$out" 2>"$err"
); then
...
echo "doltlite read path OK with GC_DOLTLITE_READ_FASTPATH=1: gc beads show $candidate"
