...
DEFAULT_BEADS_DOLTLITE_BD_REF="61fadf8c5d87f929bb47a3b23350f53b5960c1eb"
...
Options:
  ...
  --bd-ref VALUE     Expected beads-doltlite source commit for bd builds.
                     Default: 61fadf8c5d87f929bb47a3b23350f53b5960c1eb.
                     Set empty to skip the source revision check.
  --client-output FILE
...
Environment overrides:
  ...
  BD_BUILD, BD_COMMIT, BD_BRANCH, GC_BEADS_DOLTLITE_BD_REF
EOF
}
...
full_revision_for() {
  local source_dir="$1"
  if command -v jj >/dev/null 2>&1 && [ -d "$source_dir/.jj" ]; then
    (cd "$source_dir" && jj log --no-graph -r @ -T 'commit_id' 2>/dev/null | tr -d '\n' || true)
    return 0
  fi
  if command -v git >/dev/null 2>&1; then
...
build_bd() {
  ...
  if [ -n "$BD_EXPECTED_REF" ]; then
    local actual_ref
    actual_ref="$(full_revision_for "$BD_SRC")"
    if [ -z "$actual_ref" ]; then
      die "could not determine beads-doltlite source revision for $BD_SRC"
    fi
    case "$actual_ref:$BD_EXPECTED_REF" in
      "$BD_EXPECTED_REF":*|"$BD_EXPECTED_REF"*:*|*:"$actual_ref"|*:"$actual_ref"*) ;;
      *)
        die "beads-doltlite source is at $actual_ref, expected $BD_EXPECTED_REF; pass --bd-ref or set GC_BEADS_DOLTLITE_BD_REF to override"
        ;;
    esac
  fi
...
BD_EXPECTED_REF="${GC_BEADS_DOLTLITE_BD_REF-$DEFAULT_BEADS_DOLTLITE_BD_REF}"
