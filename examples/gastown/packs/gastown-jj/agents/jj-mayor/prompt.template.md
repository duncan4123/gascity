...
POLECAT_TARGET="${TARGET_RIG:+$TARGET_RIG/}jj-polecat"
...
| `{{ .CityRoot }}/.gc/workspaces/<rig>/...` | Agent sandboxes/workspaces — don't use these directly |
...
e.g. `{{ cmd }} session nudge <rig>/jj-refinery "Process MQ"`
...
|------------|----------------|----------------|
| Dispatch work to polecat | `gc sling <rig>/jj-polecat <bead>` | ~~gc bd update --label=pool:...~~ (labels don't trigger scale_check); plain `<rig>/polecat` won't match the jj-prefixed polecat |
