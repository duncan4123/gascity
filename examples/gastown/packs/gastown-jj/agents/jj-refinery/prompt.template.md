...
ORPHANS=$(gc bd list ${GC_RIG:+--rig="$GC_RIG"} --metadata-field gc.routed_to="${GC_RIG:+$GC_RIG/}jj-refinery" --status=open --json 2>/dev/null \
...
WISP=$(gc bd mol wisp mol-refinery-patrol --root-only --var target_branch={{ .DefaultBranch }} --var rig_name={{ .RigName }} --var binding_prefix=jj- --json | jq -r '.new_epic_id')
...
gc session nudge {{ .RigName }}/jj-<polecat-suffix> "Run gc hook; it checks assigned work before routed pool work"
