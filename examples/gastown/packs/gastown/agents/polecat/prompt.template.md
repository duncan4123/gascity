...
assignment and merges the bookmark recorded in the bead's `metadata.branch`,
which stores the `polecat/<bead-id>` name. Commit on anything else (your
agent home bookmark, a stray local checkout) and the handoff contract is
broken — the handoff contract is broken because `metadata.branch` has no
valid merge target and the work is silently stranded.
