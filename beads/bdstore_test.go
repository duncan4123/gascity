...
func TestBdStoreReadyWithPluralAssigneesFiltersClientSide(t *testing.T) {
	...
	if len(got) != 2 {
		t.Fatalf("Ready(plural assignees) returned %d beads, want 2", len(got))
	}
	if got[0].ID != "bd-worker-1" || got[1].ID != "bd-worker-2" {
		t.Fatalf("Ready(plural assignees) = %#v, want worker-1 then worker-2", got)
	}
}
