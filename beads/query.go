...
type ReadyQuery struct {
	Assignee string
	// Assignees matches ready work assigned to any listed assignee.
	// It is mutually exclusive with Assignee; callers should prefer Assignee
	// for the single-identity path and Assignees when batching multiple
	// assignee probes into one backend read.
	Assignees []string
	Limit     int
	// TierMode selects the storage tier(s) to read from. Zero value
...
func readyQueryAssignees(query ReadyQuery) []string {
	...
	seen := make(map[string]struct{}, len(query.Assignees))
...
func readyQueryIsZero(query ReadyQuery) bool {
	return strings.TrimSpace(query.Assignee) == "" &&
