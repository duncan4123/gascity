...
func (s *BdStore) Ready(query ...ReadyQuery) ([]Bead, error) {
	...
	now := time.Now().UTC()
	allowedAssignees := make(map[string]struct{}, len(assignees))
	for _, assignee := range assignees {
		allowedAssignees[assignee] = struct{}{}
	}
	for i := range issues {
		...
		if len(allowedAssignees) > 0 {
			if _, ok := allowedAssignees[bead.Assignee]; !ok {
				continue
			}
		} else if q.Assignee != "" && bead.Assignee != q.Assignee {
			continue
		}
