...
func (d *doltBackend) SkipBuiltinPacks() []string { return nil }
...
func (dl *doltliteBackend) SkipBuiltinPacks() []string { return []string{"dolt"} }
...
// Used by cmd_doctor.go to gate doltlite-specific checks.
func isDoltliteCity() bool {
	if v := strings.TrimSpace(os.Getenv("GC_BEADS_BACKEND")); strings.EqualFold(v, "doltlite") {
		return true
	}
	dir, err := resolveCity()
	if err != nil {
	...
	return resolveBeadsBackend(dir).Name() == "doltlite"
}
