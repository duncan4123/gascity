...
func rigProtections(rigs []resolverRig, fs fsys.FS) ([]CleanupRigProtection, []rigProtectionError) {
	...
	for _, r := range orderRigsHQFirst(rigs) {
		...
		if resolution.skip {
			// Non-dolt-backed rig (e.g. mysql): not a dolt-cleanup target,
			// so omit it from rig protections. It is then neither counted as
			// a force_blocker nor selected for forced drop/purge.
			continue
		}
...
type rigDoltDatabaseResolution struct {
	name string
	err  error
	// skip is set when the rig declares a non-dolt backend and therefore has
	// no dolt database for dolt-cleanup to verify, drop, or purge.
	skip bool
}
