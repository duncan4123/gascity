...
- No evidence in the PR head shows a replacement committed Beads configuration path for the deleted .beads/config.yaml defaults.
- No regression evidence shows pooled sessions are rebound from trigger-specific pack workspaces back to generic pool demand without retaining gc.pack, gc.pack_workspace, gc.work_dir, or legacy work_dir metadata.

## Recommended Fixes

- Restore .beads/config.yaml, or commit an equivalent configuration path that bd/gc loads in fresh checkouts, and add a regression check for the custom issue types used by Gas City.
- Clear all trigger-derived metadata when bindPoolSessionTriggerBead receives an empty WorkBeadID, including pack, pack workspace, canonical work dir, and legacy work_dir, then add a regression test for reusing a previously trigger-bound pool session as generic demand.
