...
// flags whose complete filter semantics require bd 1.0.5 or newer. DoltLite
// opts in as well so its local-store work queries can avoid the extra
// ephemeral fallback subprocesses.
func (b BeadsConfig) UsesBD105ReadySemantics() bool {
	return b.UsesBD105CLISemantics() || strings.EqualFold(strings.TrimSpace(b.Backend), "doltlite")
}
...
func bdListIncludeEphemeralArg(includeEphemeralReady bool) string {
	if includeEphemeralReady {
		return " --include-ephemeral"
	}
	return ""
...
func standardAssignedInProgressWorkQueryScript(includeEphemeralReady bool) string {
	return `for id in "$GC_SESSION_ID" "$GC_SESSION_NAME" "$GC_ALIAS"; do ` +
		`[ -z "$id" ] && continue; ` +
		`r=$(bd list` + bdListIncludeEphemeralArg(includeEphemeralReady) + ` --status in_progress --assignee="$id" --json --limit=1 2>/dev/null); ` +
		`[ -n "$r" ] && [ "$r" != "[]" ] && printf "%s" "$r" && exit 0; ` +
