...
func TestBeadsConfigUsesBD105ReadySemanticsTreatsDoltliteBackendAsCompatible(t *testing.T) {
	b := BeadsConfig{Backend: "doltlite"}
	if !b.UsesBD105ReadySemantics() {
		t.Fatalf("UsesBD105ReadySemantics() = false for doltlite backend, want true")
	}
}
...
case "$1" in
  list)
    case "$*" in
      *"list --include-ephemeral --status in_progress --assignee=hello-world/dog --json --limit=1"*)
...
func TestEffectiveWorkQueryForBeadsDoltliteUsesIncludeEphemeralReadySemantics(t *testing.T) {
	a := Agent{Name: "worker", Dir: "foundations"}
	got := a.EffectiveWorkQueryForBeads(BeadsConfig{Backend: "doltlite"})
	for _, want := range []string{
		`bd list --include-ephemeral --status in_progress --assignee="$id" --json --limit=1`,
		`bd ready --include-ephemeral --assignee="$id" --json --limit=1`,
		`bd ready --include-ephemeral --metadata-field "gc.routed_to=$target" --unassigned --exclude-type=epic --json --sort oldest --limit=1`,
	} {
		if !strings.Contains(got, want) {
			t.Fatalf("EffectiveWorkQueryForBeads(doltlite) missing %q: %q", want, got)
		}
	}
	if strings.Contains(got, "ephemeral=true AND status=in_progress") || strings.Contains(got, "ephemeral=true AND status=open") {
		t.Fatalf("EffectiveWorkQueryForBeads(doltlite) still contains ephemeral fallback probes: %q", got)
	}
}

func TestEffectivePoolDemandQueryForBeadsDoltliteUsesIncludeEphemeralReadySemantics(t *testing.T) {
	a := Agent{Name: "worker", Dir: "foundations"}
	got := a.EffectivePoolDemandQueryForBeads(BeadsConfig{Backend: "doltlite"})
	if !strings.Contains(got, `bd ready --include-ephemeral --metadata-field "gc.routed_to=$target" --unassigned --exclude-type=epic --json`) {
		t.Fatalf("EffectivePoolDemandQueryForBeads(doltlite) missing include-ephemeral routed pool probe: %q", got)
	}
	if strings.Contains(got, "ephemeral=true AND status=open") {
		t.Fatalf("EffectivePoolDemandQueryForBeads(doltlite) still contains ephemeral fallback probe: %q", got)
	}
}
