...
func workspaceListArgs() []string {
	tmpl := `name ++ "\x1F" ++ self.target().change_id().short() ++ "\x1F" ++ self.target().commit_id().short() ++ "\x1F" ++ stringify(self.target().current_working_copy()) ++ "\x1F" ++ root ++ "\n"`
...
// changes. Uses jj status to check for working-copy changes.
func (j *Jj) HasUncommittedWork() bool {
	out, err := j.runner.run(context.Background(), []string{"status", "--color", "never"})
	if err != nil {
		return true // assume dirty on error (safe default)
	}
	return hasRelevantOutput(out)
}

// hasRelevantOutput returns true if the output contains actual working-copy changes.
// Filters out jj warning, status, and summary lines — only returns true
// when "Working copy changes:" section contains file entries.
func hasRelevantOutput(out []byte) bool {
	inChanges := false
	for _, line := range strings.Split(strings.TrimSpace(string(out)), "\n") {
		...
		// Skip jj warning lines.
		if strings.HasPrefix(line, "Warning:") || strings.HasPrefix(line, "Hint:") ||
			strings.HasPrefix(line, "Failed ") || strings.HasPrefix(line, "Revision") {
			continue
		}
		// Enter the working-copy changes section.
		if strings.HasPrefix(line, "Working copy changes:") {
			inChanges = true
			continue
		}
		if inChanges {
			// Lines inside the changes section that start a new section end it.
			if strings.HasPrefix(line, "Working copy") || strings.HasPrefix(line, "Parent commit") {
				inChanges = false
				continue
			}
			// Found a real file entry (e.g., "A dirty.txt", "M foo.go").
			return true
		}
	}
	return false
