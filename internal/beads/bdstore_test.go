...
func TestBdStoreDoltliteWritesAreSerializedAcrossStores(t *testing.T) {
	dir := doltliteBdStoreTestDir(t)
	var mu sync.Mutex
	active := 0
	maxActive := 0
	runner := func(_, name string, args ...string) ([]byte, error) {
		...
		unwrapped := stripDoltliteAutoCommitArgs(args)
		if len(unwrapped) == 0 || unwrapped[0] != "update" {
			return nil, fmt.Errorf("unexpected command: bd %s", strings.Join(args, " "))
		}
	...
	for i := 0; i < 2; i++ {
		...
		go func() {
			<-start
			errs <- stores[i].SetMetadataBatch("bd-42", map[string]string{
